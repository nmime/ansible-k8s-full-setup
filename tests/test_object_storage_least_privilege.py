...
def test_nx_cache_retention_and_growth_are_enforced_by_seaweedfs():
    ...
    assert defaults["object_storage_cache_volume_compaction_schedule"] == "28 4 * * *"
    assert defaults["object_storage_cache_volume_compaction_threshold"] == "0.01"
    ...
    assert tasks.count("volume.list -collectionPattern=$collection -v=5") == 2
    assert "collectionPattern=$collection -readonly" not in tasks
