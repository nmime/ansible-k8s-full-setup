"""Contracts for the stateful Helsinki CX Telegram capacity monitor."""
...
def test_notifies_on_first_availability_and_return_but_not_unchanged(tmp_path):
    ...
    first = monitor.reconcile(
        snapshot("hel1"), state_file, "token", "chat", notify=notify
    )
    unchanged = monitor.reconcile(
        snapshot("hel1", checked_at="2026-07-25T10:15:00+00:00"),
        state_file,
        "token",
        "chat",
        notify=notify,
    ...
    returned = monitor.reconcile(
        snapshot("hel1", checked_at="2026-07-25T10:45:00+00:00"),
        state_file,
        "token",
        "chat",
        notify=notify,
...
def test_failed_notification_is_not_acknowledged_and_is_retried(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="delivery failed"):
        monitor.reconcile(snapshot("hel1"), state_file, "token", "chat", notify=fail)
...
def test_state_never_persists_credentials(tmp_path):
    ...
    assert value["notified_complete_locations"] == ["hel1"]
...
def test_partial_availability_and_each_transition_are_reported_once(tmp_path):
    ...
    unchanged = monitor.reconcile(
        partial_snapshot("hel1", "bastion", checked_at="2026-07-25T10:01:00+00:00"),
        state_file,
        "token",
        "chat",
        notify=notify,
