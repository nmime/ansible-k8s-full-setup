...
def test_ci_classification_uses_gitlab_metadata_not_named_environment_branches():
    ...
    rules = document[".platform-ci-auto"]["rules"]
    ...
    assert document[".platform-ci-auto"]["tags"] == ["ci-shared"]


def test_every_ci_class_maps_to_the_bounded_kubernetes_priority():
    ...
    rendered = "\n".join(
        ...
        for rule in document[".platform-ci-auto"]["rules"]
    )
    assert rendered.splitlines() == ["platform-ci"] * 4
...
def test_production_deploy_guard_is_protected_and_canonical():
    ...
    assert 'test "$PLATFORM_CI_CLASS" = production' in text
